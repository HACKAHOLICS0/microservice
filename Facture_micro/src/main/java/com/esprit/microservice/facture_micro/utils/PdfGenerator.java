...
public class PdfGenerator {
    ...
    public static String generateInvoicePdf(Facture facture) throws IOException, DocumentException {
        // Créez un dossier pdfs dans votre projet si ce n'est pas déjà fait
        File pdfFolder = new File("pdfs");
        if (!pdfFolder.exists()) {
            pdfFolder.mkdirs();  // Crée le dossier pdfs s'il n'existe pas
        }

        // Définir le chemin du fichier PDF
        String pdfFilePath = "pdfs/facture_" + facture.getIdFacture() + ".pdf";

        // Créez le HTML de la facture avec du CSS pour la mise en forme
        ...
        // Détails de la facture sans les colonnes "Prix Unitaire" et "Total"
        htmlContent.append("<table>");
        htmlContent.append("<tr><th>Produit</th><th>Quantité</th></tr>");

        // Ajoutez les détails des produits à la facture
        ...
        return pdfFilePath;  // Retourne le chemin du fichier généré
    }
}
