...
export class FactureComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    else if (this.session.getUser() && this.session.getUser()?.badge != 'MODERATEUR'){
  ...
  ChangeDisplay(f:Facture){
    ...
    console.log("Facture sélectionnée :", f);
  ...
  filteredFactures() {
    return this.factureList.filter(facture => {
      return facture.idFacture.toString().includes(this.searchText) ||  // Recherche par ID
             facture.montantFacture.toString().includes(this.searchText); // Recherche par Montant
    });
  }
